...
from frequencyDict import dicts
...
class Bitfinex_api(object):
    ...
    def __init__(self):
        ...
        # https://www.bitfinex.com/v2/candles/trade:30m:tBTCUSD/hist?end=1541919600000&limit=3000&_bfx=1
        ...
        self.__key = '3hour'
        self.__db = self.__client['crypto_{}'.format(self.__key)]
        self.__symbol_detail_sheet = self.__db['symbol_detail']
        self.__price_type_list = [dicts[self.__key]['price_type']]  # '5m', '15m', '30m', '1h', '3h', '6h', '12h', '1D'
        self.__step = dicts[self.__key]['step']
        self.__price_type_limit = [dicts[self.__key]['type_limit']]  # 300000, 900000, 1800000, 3600000, 10800000, 21600000, 43200000, 86400000
        self.__filePath = filename
        self.__dataFomart = data()
        self.__dataFomart.exchange = 'bitfinex'
    ...
    def create_initially_urls_queue(self):
        ...
        symbol_pairs_list = [i for i in self.get_all_symbol_detail()]
    ...
    def request_for_get_history_price_data(self, url):
        ...
        if self.symbol_replace(symbol_pair):
            symbol_pair_sheet = self.__db[self.__dataFomart.exchange + '_' + self.symbol_replace(symbol_pair)]
            symbol_pair_sheet.ensure_index([('start_time',pymongo.ASCENDING)],unique = True)
            try:
                ...
                for i in result:
                    currentT = i[0]
                    data = {}
                    data['symbol'] = self.__dataFomart.exchange + '_' + self.symbol_replace(symbol_pair)
                    data['frequency'] = self.__key
                    data['start_time'] = datetime.datetime.fromtimestamp(int(currentT)/1000)
                    data['end_time'] = datetime.datetime.fromtimestamp(int(currentT)/1000) + datetime.timedelta(seconds = self.__step)
                    data['open'] = i[1]
                    data['high'] = i[3]
                    data['low'] = i[4]
                    data['close'] = i[2]
                    data['volume'] = i[5]
                    data['amount'] = self.__dataFomart.amount
                    data['openInterest'] = self.__dataFomart.openInterest
